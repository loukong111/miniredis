...
6. 集群演示：CRC16 slot、固定 slot map、`MOVED/ASK`、slot 迁移、节点 `pfail/fail`、手动 failover takeover。
...
- GET/TTL/STRLEN 等读路径使用共享锁，SET/SETNX/APPEND/INCR/DEL/EXPIRE 使用独占锁。
...
- 收尾阶段短暂持有 AOF mutex，把 buffer 合并到临时文件，再 `fsync + rename`。
...
- 支持 `CLUSTER INFO/NODES/SLOTS/SLOTMAP/KEYSLOT/COUNTKEYSINSLOT`。
...
- master 成功执行 `SET/SETNX/APPEND/INCR/DEL/EXPIRE` 后写入 replication backlog，并通过 `REPLAPPLY <offset> ...` 转发内部复制命令。
...
3. KV 页执行 `AUTH`、`SET`、`SETNX`、`GET`、`APPEND`、`STRLEN`、`INCR`、`EXPIRE`、`TTL`、`MGET`。
...
- 实现轻量级 KV 缓存核心，支持 TTL、MGET、SETNX、APPEND、STRLEN、INCR/DECR、EXPIRE、maxmemory、LRU、64B 小对象内存池和 sharded CacheStore。
