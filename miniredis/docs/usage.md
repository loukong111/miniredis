...
`/healthz` 表示 HTTP 监控服务存活，`/readyz` 表示 RESP 服务已经 ready，可以接业务流量；`/stats` 返回 JSON，`/metrics` 返回 Prometheus text format。监控字段包含命令数、命中率、连接数、内存池、慢命令、资源保护阈值、snapshot 最近状态和 AOF rewrite 状态。
