...
public:
    ScopedTestDirectory() {
        std::string pattern =
            (std::filesystem::temp_directory_path() / "miniredis-tests-XXXXXX").string();
        std::vector<char> writable(pattern.begin(), pattern.end());
        writable.push_back('\0');
        char* created = ::mkdtemp(writable.data());
        if (!created) throw std::runtime_error("failed to create test temp directory");
        path_ = created;
    }
...
static std::filesystem::path testPath(const std::string& filename) {
    static ScopedTestDirectory directory;
...
static void testLoggerWritesFileAndFiltersLevel() {
    std::filesystem::path path = testPath("logger.log");
...
static void testConfigFileParsingAndPrecedence() {
    std::filesystem::path path = testPath("config.conf");
    {
        ...
        ofs << "automatic_failover = true\n";
        ofs << "failover_nodes = 127.0.0.1:7000,127.0.0.1:7001,127.0.0.1:7002\n";
        ofs << "failover_state_file = /tmp/miniredis_failover_from_config.state\n";
        ofs << "failover_heartbeat_interval_ms = 900\n";
        ofs << "failover_fail_threshold = 4\n";
    ...
    assert(config.failover_nodes_str ==
           "127.0.0.1:7000,127.0.0.1:7001,127.0.0.1:7002");
    assert(config.failover_state_file == "/tmp/miniredis_failover_from_config.state");
    assert(config.failover_heartbeat_interval_ms == 900);
    assert(config.failover_fail_threshold == 4);
...
static void testFilePersistence() {
    std::filesystem::path path = testPath("snapshot.dat");
...
static void testFilePersistenceLoadsV1Snapshot() {
    std::filesystem::path path = testPath("snapshot_v1.dat");
    {
...
static void testFilePersistenceRejectsHugeCount() {
    std::filesystem::path path = testPath("snapshot_bad_count.dat");
    {
        std::ofstream ofs(path, std::ios::binary | std::ios::trunc);
        ofs << "MINIREDIS_SNAPSHOT_V1\n";
...
static void testFilePersistenceBackupRecovery() {
    std::filesystem::path path = testPath("snapshot_recover.dat");
    FilePersistence persistence(path.string());
...
static void testAppendOnlyFileReplay() {
    std::filesystem::path path = testPath("appendonly.aof");
    std::filesystem::remove(path);
...
static void testAppendOnlyFileIgnoresIncompleteTail() {
    std::filesystem::path path = testPath("appendonly_bad_tail.aof");
    std::filesystem::remove(path);
...
static void testAppendOnlyFileIgnoresInterruptedRewriteTemp() {
    std::filesystem::path path = testPath("appendonly_interrupted_rewrite.aof");
...
static void testAppendOnlyFileRewriteBufferLimitAllowsRetry() {
    std::filesystem::path path = testPath("appendonly_rewrite_limit.aof");
    std::filesystem::remove(path);
...
static void testAppendOnlyFileRewrite() {
    std::filesystem::path path = testPath("appendonly_rewrite.aof");
    std::filesystem::remove(path);
    std::filesystem::remove(path.string() + ".rewrite.tmp");
...
static void testCommandHandlerWritesAof() {
    std::filesystem::path path = testPath("command_appendonly.aof");
    std::filesystem::remove(path);
...
static void testCommandHandlerRewriteAof() {
    std::filesystem::path path = testPath("command_rewrite.aof");
    std::filesystem::remove(path);
    std::filesystem::remove(path.string() + ".rewrite.tmp");
...
static void testClusterConfigStore() {
    std::filesystem::path path = testPath("cluster.conf");
    std::filesystem::remove(path);
    std::filesystem::remove(path.string() + ".tmp");
...
static void testClusterSlotTakeoverIsAtomic() {
    ClusterSlotMap slot_map;
    const std::string old_master = "127.0.0.1:6366";
    const std::string new_master = "127.0.0.1:6367";
    slot_map.Rebuild({old_master, new_master});
    const uint64_t before = slot_map.GetEpoch();
    const size_t expected = slot_map.GetSlotRangesForNode(old_master).front().end + 1;
    const size_t moved = slot_map.TakeoverNodeSlots(old_master, new_master, 42);
    assert(moved == expected);
    assert(slot_map.GetEpoch() == 42);
    assert(!slot_map.NodeOwnsSlots(old_master));
    assert(slot_map.GetNodeForSlot(0) == new_master);
    assert(slot_map.GetNodeState(old_master) == ClusterNodeState::Fail);
    assert(slot_map.GetNodeState(new_master) == ClusterNodeState::Healthy);
    assert(slot_map.TakeoverNodeSlots(old_master, new_master, before) == 0);
    assert(slot_map.GetEpoch() == 42);
}

static void testFailoverCoordinatorPersistsVotes() {
    const std::filesystem::path path = testPath("failover.state");
    std::filesystem::remove(path);
    std::filesystem::remove(path.string() + ".tmp");

    const std::string node1 = "127.0.0.1:6366";
    const std::string node2 = "127.0.0.1:6367";
    const std::string node3 = "127.0.0.1:6368";
    FailoverCoordinator coordinator(node1, path.string());
    assert(coordinator.restore());
    assert(coordinator.beginElection() == 1);
    FailoverState state = coordinator.snapshot();
    assert(state.current_epoch == 1);
    assert(state.last_voted_epoch == 1);
    assert(state.voted_for == node1);

    assert(coordinator.grantVote(1, node1, 10, 10, true));
    assert(!coordinator.grantVote(1, node2, 10, 10, true));
    assert(!coordinator.grantVote(2, node2, 9, 10, true));
    assert(!coordinator.grantVote(2, node2, 10, 10, false));
    assert(coordinator.grantVote(2, node2, 11, 10, true));
    assert(coordinator.recordLeader(2, node2));
    assert(!coordinator.recordLeader(2, node3));

    FailoverCoordinator restored(node1, path.string());
    assert(restored.restore());
    state = restored.snapshot();
    assert(state.current_epoch == 2);
    assert(state.last_voted_epoch == 2);
    assert(state.voted_for == node2);
    assert(state.leader_epoch == 2);
    assert(state.leader == node2);
    assert(!restored.grantVote(2, node3, 12, 10, true));
    assert(restored.beginElection(10) == 11);
...
static void testClusterManualFailoverTakeover() {
    ...
    AppConfig automatic_config;
    automatic_config.replicaof = master_node;
    automatic_config.automatic_failover = true;
    FixedMemoryPool automatic_pool(4);
    CacheStore automatic_cache(automatic_pool);
    ClusterSlotMap automatic_slot_map;
    automatic_slot_map.Rebuild({master_node, replica_node});
    std::mutex automatic_slot_map_mutex;
    CommandHandler automatic_handler(automatic_cache, automatic_pool, automatic_config, true,
                                     replica_node, &automatic_slot_map,
                                     &automatic_slot_map_mutex);
    bool automatic_authenticated = true;
    assert(automatic_handler.handle(command({"CLUSTER", "FAILOVER", "TAKEOVER"}),
                                    automatic_authenticated) ==
           RespWriter::error(
               "manual CLUSTER FAILOVER is disabled while automatic failover is enabled"));
    assert(automatic_slot_map.GetNodeForSlot(0) == master_node);
}
...
static void testAutomaticFailoverWriteFence() {
    FixedMemoryPool pool(8);
    CacheStore cache(pool);
    AppConfig config;
    std::atomic<bool> primary{true};
    std::atomic<bool> writes_allowed{false};
    CommandHandler handler(cache, pool, config, false, "127.0.0.1:6366", nullptr,
                           nullptr, nullptr, {}, nullptr, nullptr, {}, nullptr,
                           nullptr, {}, nullptr, nullptr, &primary, &writes_allowed);
    bool authenticated = true;

    assert(handler.handle(command({"GET", "ha:key"}), authenticated) ==
           RespWriter::nullBulkString());
    assert(handler.handle(command({"SET", "ha:key", "fenced"}), authenticated) ==
           RespWriter::error("MASTERDOWN Quorum unavailable; writes are fenced"));

    writes_allowed.store(true);
    assert(handler.handle(command({"SET", "ha:key", "accepted"}), authenticated) ==
           RespWriter::simpleString("OK"));

    primary.store(false);
    assert(handler.handle(command({"SET", "ha:key", "replica"}), authenticated) ==
           RespWriter::error("READONLY You can't write against a read only replica"));
}

static void testReplicationStateFile() {
    const std::filesystem::path path = testPath("replication.state");
    const std::string first_replid = generateReplicationId();
    const std::string second_replid = generateReplicationId();
    assert(isValidReplicationId(first_replid));
    assert(isValidReplicationId(second_replid));
    assert(first_replid != second_replid);
    assert(!isValidReplicationId("?"));
    assert(!isValidReplicationId(std::string(kReplicationIdLength, 'g')));

    assert(saveReplicationStateFile(path.string(), ReplicationState{first_replid, 42}));
    assert(!std::filesystem::exists(path.string() + ".tmp"));
    ReplicationState loaded;
    assert(loadReplicationStateFile(path.string(), loaded));
    assert(loaded.master_replid == first_replid);
    ...
    assert(loadReplicationStateFile(path.string(), loaded));
    assert(loaded.master_replid.empty());
    assert(loaded.offset == 17);

    {
        std::ofstream corrupt(path, std::ios::trunc);
        corrupt << "MINIREDIS_REPL_STATE_V1\nmaster_replid invalid\noffset 9\n";
    }
    assert(!loadReplicationStateFile(path.string(), loaded));
    assert(loaded.master_replid.empty());
...
static void testCommandReplicationBacklogPsync() {
    ...
    AppConfig config;
    AppConfig replica_config;
    replica_config.replicaof = "127.0.0.1:6366";
    ReplicationBacklog backlog(8);
    std::atomic<uint64_t> replica_offset{0};
    uint64_t persisted_offset = 0;
    std::mutex replication_mutex;
    const std::string master_replid(kReplicationIdLength, 'a');
    const std::string restarted_master_replid(kReplicationIdLength, 'b');
    const std::string replica_local_replid(kReplicationIdLength, 'c');
    std::string upstream_replid = master_replid;
    CommandHandler master(cache, pool, config, false, "127.0.0.1:6366", nullptr, nullptr,
                          nullptr, {}, &backlog, nullptr, {}, nullptr, nullptr, {},
                          &master_replid);
    CommandHandler replica(cache, pool, replica_config, false, "127.0.0.1:6367", nullptr, nullptr,
                           nullptr, {}, nullptr, &replica_offset,
                           [&](uint64_t offset) { persisted_offset = offset; }, nullptr,
                           &replication_mutex, {}, &replica_local_replid, &upstream_replid);
    bool authenticated = true;
    ...
    std::string psync = master.handle(
        command({"REPLPSYNC", master_replid, "0"}), authenticated);
    RespDecoder decoder;
    decoder.feed(psync);
    auto parsed = decoder.parse();
    assert(parsed.has_value());
    assert(parsed->type == RespType::ARRAY);
    assert(parsed->array.size() == 4);
    assert(parsed->array[0].str == "CONTINUE");
    assert(parsed->array[1].str == master_replid);
    assert(parsed->array[2].integer == 2);
    assert(parsed->array[3].type == RespType::ARRAY);
    assert(parsed->array[3].array.size() == 2);
    assert(parsed->array[3].array[0].array[0].integer == 1);
    assert(parsed->array[3].array[0].array[1].array[0].str == "REPLSET");

    assert(replica.handle(command({"REPLAPPLY", master_replid, "2", "REPLSET",
                                   "applied", "gap", "0"}),
                          authenticated) ==
           RespWriter::error("REPLGAP expected offset 1 but got 2"));
    assert(replica.handle(command({"REPLAPPLY", restarted_master_replid, "1", "REPLSET",
                                   "applied", "wrong-master", "0"}), authenticated) ==
           RespWriter::error("REPLID mismatch; full resync required"));
    assert(replica.handle(command({"REPLAPPLY", master_replid, "1", "REPLSET",
                                   "applied", "ok", "0"}),
                          authenticated) == RespWriter::simpleString("OK"));
    assert(replica_offset.load() == 1);
    assert(persisted_offset == 1);
    assert(replica.handle(command({"REPLACK", master_replid}), authenticated) ==
           RespWriter::integer(1));
    assert(replica.handle(command({"REPLACK", restarted_master_replid}), authenticated) ==
           RespWriter::error("REPLID mismatch; full resync required"));
    assert(replica.handle(command({"REPLAPPLY", master_replid, "1", "REPLSET",
                                   "applied", "duplicate", "0"}),
                          authenticated) == RespWriter::simpleString("OK"));
    ...
    assert(parsed->type == RespType::ARRAY);
    assert(parsed->array.size() == 4);
    assert(parsed->array[0].str == "FULLRESYNC");
    assert(parsed->array[1].str == master_replid);
    assert(parsed->array[2].integer == 2);
    assert(parsed->array[3].type == RespType::ARRAY);

    CommandHandler restarted_master(cache, pool, config, false, "127.0.0.1:6366",
                                    nullptr, nullptr, nullptr, {}, &backlog, nullptr, {},
                                    nullptr, nullptr, {}, &restarted_master_replid);
    std::string identity_mismatch = restarted_master.handle(
        command({"REPLPSYNC", master_replid, "2"}), authenticated);
    RespDecoder mismatch_decoder;
    mismatch_decoder.feed(identity_mismatch);
    auto mismatch = mismatch_decoder.parse();
    assert(mismatch.has_value());
    assert(mismatch->type == RespType::ARRAY);
    assert(mismatch->array.size() == 3);
    assert(mismatch->array[0].str == "FULLRESYNC");
    assert(mismatch->array[1].str == restarted_master_replid);
    assert(mismatch->array[2].integer == 2);

    std::string repl_info = replica.handle(command({"INFO", "replication"}), authenticated);
    assert(repl_info.find("master_replid:" + master_replid) != std::string::npos);
}
...
static void testCommandMetadata() {
    ...
    for (const auto& entry : parsed->array) {
        ...
        if (entry.array[0].str == "replfailover") {
            saw_replfailover = true;
            assert(entry.array[1].integer == -2);
        }
...
static void testStatsReadinessExport() {
    ...
    Stats::instance().setFailoverState(true, 3, 2, 2, 7, 7, true, false);
    ...
    assert(json.find("\"failover_group_size\":3") != std::string::npos);
    assert(json.find("\"failover_current_epoch\":7") != std::string::npos);
    assert(json.find("\"failover_master_failed\":true") != std::string::npos);
    assert(json.find("\"failover_writes_allowed\":false") != std::string::npos);
    ...
    assert(metrics.find("miniredis_failover_quorum 2") != std::string::npos);
    assert(metrics.find("miniredis_failover_reachable_nodes 2") != std::string::npos);
    assert(metrics.find("miniredis_failover_elections_total 1") != std::string::npos);
...
int main() {
    ...
    testClusterSlotTakeoverIsAtomic();
    testFailoverCoordinatorPersistsVotes();
    testClusterCommands();
    ...
    testAutomaticFailoverWriteFence();
    testReplicationStateFile();
