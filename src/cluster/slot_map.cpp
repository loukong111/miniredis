...
size_t ClusterSlotMap::TakeoverNodeSlots(const std::string& old_owner,
                                         const std::string& new_owner,
                                         uint64_t minimum_epoch) {
    std::lock_guard<std::mutex> lock(mutex_);
    if (old_owner.empty() || new_owner.empty() || old_owner == new_owner ||
        std::find(nodes_.begin(), nodes_.end(), old_owner) == nodes_.end() ||
        std::find(nodes_.begin(), nodes_.end(), new_owner) == nodes_.end()) {
        return 0;
    }

    size_t moved = 0;
    for (size_t slot = 0; slot < slot_owner_.size(); ++slot) {
        if (slot_owner_[slot] != old_owner) continue;
        slot_owner_[slot] = new_owner;
        slot_meta_[slot] = ClusterSlotMeta{};
        ++moved;
    }
    if (moved == 0) return 0;

    node_states_[old_owner] = ClusterNodeState::Fail;
    node_states_[new_owner] = ClusterNodeState::Healthy;
    if (epoch_ < std::numeric_limits<uint64_t>::max()) ++epoch_;
    epoch_ = std::max(epoch_, minimum_epoch);
    return moved;
}
