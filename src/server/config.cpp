...
bool getEnvBoolOr(const char* name, bool fallback) {
    const char* value = std::getenv(name);
    return value ? parseBool(value) : fallback;
}
...
void applyConfigValue(AppConfig& config, const std::string& raw_key, const std::string& raw_value) {
    ...
    else if (key == "automatic_failover") config.automatic_failover = parseBool(value);
    else if (key == "failover_nodes" || key == "failover_nodes_str") config.failover_nodes_str = value;
    else if (key == "failover_state_file") config.failover_state_file = value;
    else if (key == "failover_heartbeat_interval_ms") config.failover_heartbeat_interval_ms = parseSizeStrict(value, "failover_heartbeat_interval_ms");
    else if (key == "failover_fail_threshold") config.failover_fail_threshold = parseIntStrict(value, "failover_fail_threshold");
...
void loadEnvironment(AppConfig& config) {
    ...
    config.automatic_failover = getEnvBoolOr("MINIREDIS_AUTOMATIC_FAILOVER", config.automatic_failover);
    config.failover_nodes_str = getEnvOr("MINIREDIS_FAILOVER_NODES", config.failover_nodes_str);
    config.failover_state_file = getEnvOr("MINIREDIS_FAILOVER_STATE_FILE", config.failover_state_file);
    config.failover_heartbeat_interval_ms = getEnvSizeOr("MINIREDIS_FAILOVER_HEARTBEAT_INTERVAL_MS", config.failover_heartbeat_interval_ms);
    config.failover_fail_threshold = getEnvIntOr("MINIREDIS_FAILOVER_FAIL_THRESHOLD", config.failover_fail_threshold);
...
void printUsage(const char* program) {
    std::cerr
        ...
        << "  --automatic-failover     Enable quorum-based replication failover\n"
        << "  --failover-nodes list    Full replication group: master,replica1,replica2\n"
        << "  --failover-state-file path  Persist election epoch, vote, and leader\n"
        << "  --failover-heartbeat-interval-ms ms  Failure detector interval (default: 1000)\n"
        << "  --failover-fail-threshold count  Failed heartbeats before election (default: 3)\n"
...
ConfigParseResult parseConfig(int argc, char* argv[], AppConfig& config) {
    ...
    static struct option long_options[] = {
        ...
        {"automatic-failover", no_argument, 0, 1034},
        {"failover-nodes", required_argument, 0, 1035},
        {"failover-state-file", required_argument, 0, 1036},
        {"failover-heartbeat-interval-ms", required_argument, 0, 1037},
        {"failover-fail-threshold", required_argument, 0, 1038},
    ...
    try {
        while ((opt = getopt_long(argc, argv, "ca:n:b:p:s:i:r:h", long_options, nullptr)) != -1) {
            switch (opt) {
                ...
                case 1034: config.automatic_failover = true; break;
                case 1035: config.failover_nodes_str = optarg; break;
                case 1036: config.failover_state_file = optarg; break;
                case 1037: config.failover_heartbeat_interval_ms = parseSizeStrict(optarg, "failover-heartbeat-interval-ms"); break;
                case 1038: config.failover_fail_threshold = parseIntStrict(optarg, "failover-fail-threshold"); break;
    ...
    if (config.failover_heartbeat_interval_ms < 100 ||
        config.failover_heartbeat_interval_ms > 60000 ||
        config.failover_fail_threshold <= 0 || config.failover_fail_threshold > 100) {
        std::cerr << "failover heartbeat must be in range 100..60000 ms and fail threshold in range 1..100"
                  << std::endl;
        return ConfigParseResult::Error;
    }
    if (config.automatic_failover && config.failover_nodes_str.empty()) {
        std::cerr << "automatic failover requires --failover-nodes" << std::endl;
        return ConfigParseResult::Error;
    }
