...
ReplicationDispatcher::ReplicationDispatcher(ReplicationBacklog& backlog,
    ...
    : backlog_(backlog),
      master_replid_(std::move(master_replid)),
      password_(std::move(password)),
      reconnect_delay_(std::max(reconnect_delay, std::chrono::milliseconds(50))) {
    if (!isValidReplicationId(master_replid_)) {
        throw std::invalid_argument("invalid master replication ID");
    }
...
bool ReplicationDispatcher::setMasterReplicationId(std::string replid) {
    if (running_.load(std::memory_order_acquire) || !isValidReplicationId(replid)) {
        return false;
    }
    master_replid_ = std::move(replid);
    return true;
}
...
void ReplicationDispatcher::workerLoop(Worker& worker) {
    while (running_.load(std::memory_order_acquire)) {
        ...
        bool replid_mismatch = false;
        if (!queryReplicaOffset(worker, remote_offset, replid_mismatch)) {
            recordFailure(worker, replid_mismatch
                                      ? "replica replication ID mismatch; full sync required"
                                      : "replica ACK failed");
            disconnect(worker);
            std::unique_lock<std::mutex> lock(worker.wait_mutex);
            worker.wakeup.wait_for(lock, reconnect_delay_, [this]() {
...
bool ReplicationDispatcher::queryReplicaOffset(Worker& worker, uint64_t& offset,
                                               bool& replid_mismatch) {
    replid_mismatch = false;
    const int fd = worker.fd.load(std::memory_order_acquire);
    if (fd < 0 || !writeAll(fd, encodeRespCommand({"REPLACK", master_replid_}))) {
        return false;
    }
    std::string response;
    if (!readLine(fd, response)) return false;
    replid_mismatch = response.rfind("-ERR REPLID", 0) == 0;
    return parseIntegerResponse(response, offset);
}

bool ReplicationDispatcher::sendEntry(Worker& worker,
                                      const ReplicationBacklogEntry& entry) {
    std::vector<std::string> command;
    command.reserve(entry.command.size() + 3);
    command.push_back("REPLAPPLY");
    command.push_back(master_replid_);
