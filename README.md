...
- 可选 Qt Console 演示工具，支持常用命令测试、Cluster 查询和运行指标展示
- 默认本机绑定，可选 AUTH
...
stats HTTP server 是轻量同步实现，面向健康检查和低频监控；`/stats` 返回 JSON，`/metrics` 返回 Prometheus text format。请求读取会循环到 HTTP header 完整，响应写回会循环直到全部发送完成，并对过大的请求头返回 431。
...
`tools/qt_console` 提供一个可选的 Qt Widgets 演示工具，用于在面试或本地调试时快速验证 MiniRedis：

- 连接 RESP 端口并自动执行 `AUTH`
- 测试 `PING`、`SET`、`GET`、`MGET`、`DEL`、`EXPIRE`、`TTL`
- 查询 `CLUSTER INFO`、`CLUSTER NODES`、`CLUSTER KEYSLOT`、`CLUSTER SLOTS`
...
- 查询 `/stats` 和 `/metrics`，验证 key 数、内存池指标和命中率字段
- 停服后重启，验证快照恢复
- 启动单节点 cluster 模式
- 验证 `CLUSTER INFO`、`CLUSTER NODES`、`CLUSTER KEYSLOT`、`CLUSTER SLOTS`
...
- 支持 `CLUSTER INFO`、`CLUSTER NODES`、`CLUSTER KEYSLOT key`、`CLUSTER SLOTS`，`CLUSTER NODES` 会展示节点负责的 slot 范围
...
- stats 仍是基础指标，HTTP 接口只暴露平均/最大命令延迟，尚未提供延迟直方图或 p95/p99 分位数
- 集群模式是实验性能力
- 没有 AOF、主从复制和自动故障转移
