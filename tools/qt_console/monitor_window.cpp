...
bool MonitorWindow::exportScreenshots(const QString& directory) {
    ...
    if (demo_output_) {
        demo_output_->setPlainText(
            ...
            "> INFO replication\n"
            "role:master\n"
            "master_replid:9f5c...40位生命周期ID\n"
            "master_repl_offset:42\n"
            "repl_backlog_active:1\n"
            "步骤 2：停止副本 6367，主节点继续接收写入\n"
            "步骤 3：重启副本，使用保存的 replid + offset 进行 REPLPSYNC\n"
    ...
    if (metrics_output_) {
        metrics_output_->setPlainText(
            ...
            "miniredis_failover_enabled 1\n"
            "miniredis_failover_quorum 2\n"
            "miniredis_failover_reachable_nodes 3\n"
            "miniredis_failover_current_epoch 4\n"
            "miniredis_failover_writes_allowed 1\n");
    }
    if (benchmark_output_) {
        benchmark_runs_spin_->setValue(3);
        benchmark_redis_baseline_check_->setChecked(true);
        ...
        benchmark_output_->setPlainText(
            "> RUNS=3 BASELINE_REDIS=1 scripts/benchmark.sh\n"
            "\n"
            ...
            "Redis      64B    SET      36857.90   35100.04   3.94%   1.762\n"
            "Redis      64B    GET      35522.03   34891.84   1.35%   2.242\n"
            "\n"
            "报告: build/benchmark-report.txt\n"
            "逐轮数据: build/benchmark-runs.tsv\n");
    }
...
void MonitorWindow::buildMenus() {
    ...
    QAction* failover_script_action = tools_menu->addAction("运行自动故障转移验证");
    ...
    connect(failover_script_action, &QAction::triggered, this, [this]() {
        runScriptTool("scripts/failover_demo.sh", {"smoke"});
        openToolDialog(1, "演示中心");
    });
...
QWidget* MonitorWindow::buildDemoLabTab() {
    ...
    QPushButton* failover_script_btn = new QPushButton("自动故障转移", script_box);
    ...
    script_grid->addWidget(failover_script_btn, 1, 1);
    script_grid->addWidget(cluster_script_btn, 2, 0, 1, 2);
    script_grid->addWidget(stop_script_btn, 3, 0, 1, 2);
    ...
    connect(failover_script_btn, &QPushButton::clicked, this, [this]() {
        runScriptTool("scripts/failover_demo.sh", {"smoke"});
    });
...
QWidget* MonitorWindow::buildServerTab() {
    ...
    failover_nodes_edit_ = new QLineEdit("127.0.0.1:6366,127.0.0.1:6367,127.0.0.1:6368", config_box);
    failover_nodes_edit_->setPlaceholderText("至少三个投票节点，使用逗号分隔");
    ...
    failover_heartbeat_spin_ = new QSpinBox(config_box);
    failover_heartbeat_spin_->setRange(100, 60000);
    failover_heartbeat_spin_->setValue(1000);
    failover_heartbeat_spin_->setSuffix(" ms");
    failover_threshold_spin_ = new QSpinBox(config_box);
    failover_threshold_spin_->setRange(1, 100);
    failover_threshold_spin_->setValue(3);
    ...
    automatic_failover_check_ = new QCheckBox("自动故障转移", config_box);
    ...
    cluster_grid->addWidget(automatic_failover_check_, 4, 0);
    cluster_grid->addWidget(failover_nodes_edit_, 4, 1, 1, 3);
    cluster_grid->addWidget(makeFieldLabel("探测间隔"), 5, 0);
    cluster_grid->addWidget(failover_heartbeat_spin_, 5, 1);
    cluster_grid->addWidget(makeFieldLabel("失败阈值"), 5, 2);
    cluster_grid->addWidget(failover_threshold_spin_, 5, 3);
...
QWidget* MonitorWindow::buildBenchmarkTab() {
    ...
    benchmark_runs_spin_ = new QSpinBox(page);
    benchmark_runs_spin_->setRange(1, 10);
    benchmark_runs_spin_->setValue(3);
    benchmark_redis_baseline_check_ = new QCheckBox("Redis 同机参考", page);
    benchmark_status_label_ = new QLabel("空闲", page);
    benchmark_status_label_->setTextInteractionFlags(Qt::TextSelectableByMouse);

    QPushButton* run_btn = new QPushButton("当前服务单轮压测", page);
    QPushButton* matrix_btn = new QPushButton("运行多轮矩阵", page);
    ...
    controls->addWidget(new QLabel("脚本轮数", page), 1, 0);
    controls->addWidget(benchmark_runs_spin_, 1, 1);
    controls->addWidget(benchmark_redis_baseline_check_, 1, 2, 1, 2);
    controls->addWidget(new QLabel("状态", page), 1, 4);
    controls->addWidget(benchmark_status_label_, 1, 5);
    controls->addWidget(run_btn, 2, 0, 1, 2);
    controls->addWidget(matrix_btn, 2, 2);
    controls->addWidget(stop_btn, 2, 3);
    controls->addWidget(clear_btn, 2, 4, 1, 2);
...
void MonitorWindow::startServer() {
    ...
    if (automatic_failover_check_->isChecked()) {
        const QString failover_nodes = failover_nodes_edit_->text().trimmed();
        const QString node_addr = node_addr_edit_->text().trimmed();
        if (failover_nodes.isEmpty() || node_addr.isEmpty()) {
            appendServerLog("ERR 自动故障转移需要当前节点地址和至少三个投票节点");
            return;
        }
        args << "--automatic-failover"
             << "--failover-nodes" << failover_nodes
             << "--failover-heartbeat-interval-ms"
             << QString::number(failover_heartbeat_spin_->value())
             << "--failover-fail-threshold"
             << QString::number(failover_threshold_spin_->value())
             << "--node-addr" << node_addr;
    }
    ...
    if (cluster_check_->isChecked()) {
        args << "--cluster";
        if (!automatic_failover_check_->isChecked() &&
            !node_addr_edit_->text().trimmed().isEmpty()) {
            args << "--node-addr" << node_addr_edit_->text().trimmed();
        }
...
void MonitorWindow::runBenchmarkMatrix() {
    ...
    env.insert("RUNS", QString::number(benchmark_runs_spin_->value()));
    env.insert("BASELINE_REDIS", benchmark_redis_baseline_check_->isChecked() ? "1" : "0");
    ...
    appendBenchmarkLog(
        QString("> RUNS=%1 BASELINE_REDIS=%2 BENCH_MATRIX=1 scripts/benchmark.sh")
            .arg(benchmark_runs_spin_->value())
            .arg(benchmark_redis_baseline_check_->isChecked() ? 1 : 0));
...
void MonitorWindow::runReplicationPsyncDemo() {
    ...
    QTimer::singleShot(1800, this, [this, connect_host, master_port]() {
        appendDemoLog("步骤 1：向主节点写入数据，并查看 replication ID + offset");
        resp_client_.connectToServer(connect_host, static_cast<quint16>(master_port));
    ...
    QTimer::singleShot(5600, this, [this, program, bind, connect_host, replica_port,
        ...
        appendDemoLog("步骤 3：重启副本，使用保存的 replid + offset 进行 REPLPSYNC");
        if (program.isEmpty()) {
